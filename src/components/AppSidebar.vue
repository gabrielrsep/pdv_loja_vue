  ...
  <aside :class="[
    ...
    <div class="p-6 pb-2">
      <div class="flex items-center space-x-3 mb-1 overflow-hidden whitespace-nowrap">
        <div
          class="w-8 h-8 bg-primary-600 rounded-lg flex-shrink-0 flex items-center justify-center shadow-lg shadow-primary-600/20">
        ...
        <h1 v-show="!collapsed" class="text-lg font-bold tracking-tight text-slate-800 transition-opacity">Rangel Modas
        </h1>
      </div>
      <p v-show="!collapsed"
        class="text-[10px] text-slate-400 font-semibold tracking-widest uppercase ml-11 transition-opacity">{{
    ...
    <nav class="flex-1 px-3 py-4 space-y-6 overflow-y-auto custom-scrollbar">
      <!-- Seção Operacional -->
      <div class="space-y-1">
        <p v-show="!collapsed" class="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">
          Operacional</p>

        <router-link to="/pos" class="group flex items-center space-x-3 p-3 rounded-xl transition-all duration-200"
          ...
          <svg xmlns="http://www.w3.org/2000/svg"
            class="w-5 h-5 flex-shrink-0 transition-transform group-hover:scale-110" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="space-y-1">
        ...
        <router-link v-if="authStore.isAdmin" to="/categories"
          class="group flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 text-slate-500 hover:bg-slate-50 hover:text-slate-900"
          active-class="bg-primary-50 text-primary-600 shadow-sm">
          <TagIcon />
        ...
        <router-link v-if="authStore.isAdmin" to="/promotions"
          class="group flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 text-slate-500 hover:bg-slate-50 hover:text-slate-900"
          active-class="bg-primary-50 text-primary-600 shadow-sm">
          <TicketPercentIcon />
      ...
      <div v-if="authStore.isAdmin" class="space-y-1">
        <p v-show="!collapsed" class="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Sistema
        </p>
    ...
    <div v-show="!collapsed"
      class="group relative p-4 text-slate-400 flex flex-col items-center justify-center cursor-help transition-colors hover:text-slate-600">
      <span class="text-[10px]">{{ appInfo.version }}</span>
      <span class="text-[10px]">Copyright &#169; 2026 {{ appInfo.author }}</span>

      <!-- Tooltip -->
      <div
        class="absolute left-[10px] bottom-full w-[200px] mb-2 px-3 py-1.5 bg-slate-800 text-white text-[10px] font-medium rounded-lg opacity-0 translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200 pointer-events-none whitespace-nowrap shadow-xl z-50">
        <p class="text-wrap">This program is free software: you can redistribute it and/or modify
      ...
      <div :class="[
        ...
        <div v-show="!collapsed" class="flex flex-col min-w-0">
          ...
          <button @click="handleLogout"
            class="text-[10px] text-red-500 hover:text-red-700 font-semibold uppercase tracking-wider text-left">
            Logout
          </button>
        </div>

        <button v-show="collapsed" @click="handleLogout"
        ...
        <div class="w-8 h-8 rounded-full bg-slate-100 flex-shrink-0 flex items-center justify-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-slate-500" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
